<template>
  <div
    class="w-full control-card-bg shadow shadow-black/5 rounded-xl"
    :class="{ 'opacity-50 hover:opacity-100 duration-200': hidden }"
    @mousemove="gradientBorder($event)"
    data-card
  >
    <div class="m-px divide-y rounded-[11px]">
      <div v-if="image" class="p-3 flex bg-white control-pattern-lines rounded-t-[11px]">
      ...
      <div class="p-3 flex-1 font-medium bg-white truncate max-w-full" :class="{ 'rounded-t-[11px]': !image }" :title>
        {{ title }}
      </div>

      <div class="control-details p-3 flex bg-white flex-wrap gap-3">
        <slot />
      ...
      <div
        ...
        class="flex *:flex-1 *:p-2 *:duration-200 divide-x *:text-sm text-center rounded-b-[11px] overflow-hidden bg-white/95"
      >
        <slot name="actions">
          <NuxtLink :to="actionEdit" class="flex gap-2 items-center justify-center">
          ...
          <button
            type="button"
            @click="actionDelete"
            class="hover:bg-red-50/40 hover:text-red-500 flex gap-2 items-center justify-center"
          >
