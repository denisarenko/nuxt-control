<template>
  <div v-if="editor.isActive('image')" :class="classColor">
    <div class="flex items-center gap-2 p-1.5">
      <FormEditorIcon name="image-alt" class="size-4 shrink-0" />
    ...
    <div class="flex justify-between gap-1.5 p-1.5">
      <template v-for="tool in toolbar" :key="tool.icon">
        <button
          v-if="!tool.divide"
          type="button"
          :class="[classButton, { 'ring-blue-500': editor.getAttributes('image').class === tool.action }]"
          @click="setImage(tool.action)"
        >
          <FormEditorIcon :name="tool.icon" class="size-4" />
        </button>
