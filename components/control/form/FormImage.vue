<template>
  <div class="bg-white ring-1 ring-slate-200 shadow relative control-pattern-lines rounded-xl p-5">
    <input type="file" class="absolute inset-0 cursor-pointer opacity-0" @change="handleImage($event)" />

    <img v-if="image || preview" :src="image || preview" alt="Image" class="mx-auto max-h-52 rounded-2xl shadow" />
