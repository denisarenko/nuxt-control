<template>
  <div class="bg-slate-50 min-h-dvh bg-dot text-slate-800">
    <div class="max-w-7xl mx-auto px-4">
      <header class="py-8 flex justify-between items-center">
        ...
        <div v-if="hasAuth" class="flex gap-5 items-center">
          ...
          <button
            type="button"
            @click="logout"
            class="bg-white ring-1 ring-slate-200 rounded-full size-9 flex shadow"
          >
            <svg
      ...
      <main
        v-if="hasAuth"
        class="grid grid-cols-[250px_1fr] items-start gap-8 pb-8"
      >
        <aside
          class="bg-white ring-1 ring-slate-200 shadow grid gap-2 rounded-3xl p-4 sticky top-5"
        >
          <template v-for="menu in menuLinks" :key="menu">
            <NuxtLink
              v-if="!menu.divide"
              :to="menu.link"
              class="px-4 py-2 hover:bg-slate-100 rounded-lg duration-200"
            >
              {{ menu.name }}
            </NuxtLink>
            ...
            <div
              v-else
              class="uppercase text-xs px-4 font-semibold opacity-50 my-2"
            >
              {{ menu.name }}
            </div>
          </template>
      ...
      <section
        v-else
        class="bg-white ring-1 ring-slate-200 shadow rounded-3xl p-5 md:py-14"
      >
        <slot name="auth" />
      </section>
    </div>
